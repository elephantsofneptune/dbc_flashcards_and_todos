...
class Controller
  ...
  def deck_manager
    deck_files = {"Dev Questions" => 'flashcards.csv', 
      "Arrested Development" => 'AD_trivia.csv'}
    puts "Please choose a deck:"
    deck_files.each {|file| puts file[0]} 
    chosen_deck = gets.chomp
    @my_DB = DatabaseReader.new(deck_files[chosen_deck])
